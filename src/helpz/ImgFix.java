...
public class ImgFix {
    ...
    //Rotate Secend Img Only+ animation
    public static BufferedImage[] getBuildRotImg(BufferedImage[] imgs, BufferedImage secendImg, int rotAngle) {
        int w = imgs[0].getWidth();
        int h = imgs[0].getHeight();
        ...
        for (int i = 0; i < imgs.length; i++) {
            ...
            g2d.drawImage(imgs[i], 0, 0, null);
            g2d.rotate(Math.toRadians(rotAngle), w / 2, h / 2);
            g2d.drawImage(secendImg, 0, 0, null);
            g2d.dispose();
            arr[i]=newImg;
        }
